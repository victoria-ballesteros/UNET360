...
from core.dtos.node_dto import NodeCreateDTO, NodeUpdateDTO
...
service = NodeService(
    NodeRepository(), LocationRepository(), TagRepository(), graph_adapter
)
...
async def search_nodes_by_keywords(
    q: Optional[str] = Query(
        None, min_length=3, description="Space-separated search keywords"
    ),
):
    if not q:
        return GeneralResponse(
            ...
            status=False,
            response_obj={
                "message": "Query parameter 'q' is required and must have at least 3 characters."
            },
        )
    try:
...
async def delete_node(name: str, _: str = Depends(get_current_admin_user)):
    try:
