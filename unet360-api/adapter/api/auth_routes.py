...
from core.dtos.auth_dto import (
    UserSignUpDTO,
    UserLoginDTO,
    ForgotPasswordDTO,
    ResetPasswordDTO,
...
async def signup_user(
    user_data: UserSignUpDTO, auth_service: AuthService = Depends(get_auth_service)
):
    try:
        signup_result = await auth_service.signup_user(user_data)

        return GeneralResponse(
            http_code=status.HTTP_201_CREATED, status=True, response_obj=signup_result
        )
...
async def login_user(
    user_data: UserLoginDTO, auth_service: AuthService = Depends(get_auth_service)
):
    try:
...
async def forgot_password(
    dto: ForgotPasswordDTO, auth_service: AuthService = Depends(get_auth_service)
):
    try:
        ...
        logger.error(
            f"An unexpected error occurred during forgot password in route: {e}"
        )
        traceback.print_exc()
        return GeneralResponse(
            http_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_password(
    dto: ResetPasswordDTO, auth_service: AuthService = Depends(get_auth_service)
):
    try:
...
async def get_auth_status(request: Request):
    try:
        ...
        return GeneralResponse(
            ...
            status=False,
            response_obj={"is_authenticated": False, "message": e.detail},
        )
    except Exception as e:
...
async def logout_user(
    ...
    try:
        ...
        return GeneralResponse(
            http_code=status.HTTP_200_OK, status=True, response_obj=logout_result
        )
